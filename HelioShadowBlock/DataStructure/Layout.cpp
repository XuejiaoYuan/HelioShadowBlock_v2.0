...
void Layout::adjustHelioLayout(vector<Heliostat*>& helios, const vector<vector<float>*>& field_args, const vector<Receiver*>& recvs)
{
	...
	// 构架矩形镜场排布
	// 1. 调整Layout的各个参数
	helio_interval = Vector3f((*field_args[0])[0], (*field_args[0])[1], (*field_args[0])[2]);			// 定日镜间隔
	float z_start = (*field_args[1])[0];					// 定日镜第一行距离与接收器之间距离
...
{
	float dsep = (*field_args[0])[0];					// 定日镜包围盒安全距离
	float dm = sqrt(pow(helio_size.x(), 2) + pow(helio_size.y(), 2))+ dsep;	// 定日镜对角线长度
	float helio_recv_dis1 = (*field_args[1])[0];		// 第一个同心圆与接收器之间的距离
	float helio_recv_dis2 = 2 * helio_recv_dis1;		// 第二个同心环最内圈圆与接收器之间的距离
	float helio_recv_dis3 = 4 * helio_recv_dis1;		// 第三个同心环最内圈圆与接收器之间的距离
	float helio_recv_dis4 = 8 * helio_recv_dis1;		// 第四个同心环最内圈圆与接收器之间的距离
	float helio_gap1 = (*field_args[2])[0] * dm;				// 第一个同心环中定日镜分布间隔
	float helio_gap2 = (*field_args[3])[0] * dm;				// 第二个同心环中定日镜分布间隔
	float helio_gap3 = (*field_args[4])[0] * dm;				// 第三个同心换中定日镜分布间隔
	float angle_delta1 = dm / helio_recv_dis1;			// 第一个同心圆环定日镜摆放角度间隔
	float angle_delta2 = angle_delta1 / 2;				// 第二个同心环定日镜摆放角度间隔
	float angle_delta3 = angle_delta1 / 4;				// 第三个同心环定日镜摆放角度间隔
	...
	helio_interval = Vector3f(dm, dm, dm);				// 定日镜包围盒间距
